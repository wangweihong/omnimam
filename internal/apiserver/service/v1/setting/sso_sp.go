...
func (s *settingService) ServiceProviderGetByKey(ctx context.Context, key string) (*iapiserver.ServiceProviderGetResponse, error) {
	metas, _, err := s.store.ServiceProviders().List(ctx, &iapiserver.ServiceProviderListRequest{Protocol: iapiserver.SSOProtocolSAML})
	if err != nil {
		return nil, errors.WithStack(err)
	}
...
func (s *settingService) ServiceProviderGetRedirectURL(ctx context.Context, req *iapiserver.ServiceProviderGetRequest) (*iapiserver.ServiceProviderGetRedirectURLResponse, error) {
	meta, err := s.store.ServiceProviders().Get(ctx, req.ID)
	if err != nil {
		return nil, errors.WithStack(err)
	}
